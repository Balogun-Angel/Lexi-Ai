import { Link } from 'react-router-dom'
import { Logo } from '../components/ui/Logo'
import { Button } from '../components/ui/Button'
import { Input } from '../components/ui/Input'
import { Card } from '../components/ui/Card'

function AuthIllustration() {
  return (
    <div className="relative hidden flex-col items-center justify-center p-12 lg:flex">
      <div className="relative mb-12">
        <div className="absolute inset-0 rounded-full bg-primary/20 blur-3xl" />
        <div className="relative flex items-center gap-8">
          <div className="space-y-3">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="flex h-12 w-10 items-center justify-center rounded border border-border/50 bg-surface-card"
              >
                <span className="text-[10px] font-bold text-red-400">PDF</span>
              </div>
            ))}
          </div>
          <div className="relative">
            <div className="h-32 w-32 rounded-full bg-gradient-to-br from-primary via-accent-purple to-accent-cyan opacity-80 blur-sm" />
            <div className="absolute inset-0 flex items-center justify-center">
              <svg viewBox="0 0 64 64" className="h-24 w-24 text-primary">
                <path
                  fill="currentColor"
                  opacity="0.6"
                  d="M32 8c-8 4-14 12-14 22 0 12 6 22 14 26 8-4 14-14 14-26S40 12 32 8z"
                />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <h2 className="max-w-sm text-center text-3xl font-bold text-text-primary">
        Turn documents into conversations.
      </h2>
    </div>
  )
}

export function LoginPage() {
  return (
    <div className="flex min-h-screen bg-surface">
      <AuthIllustration />

      <div className="flex flex-1 items-center justify-center p-6">
        <Card className="w-full max-w-md" glow padding="lg">
          <div className="mb-8 flex justify-center">
            <Logo />
          </div>

          <h1 className="mb-8 text-center text-2xl font-bold text-text-primary">Welcome Back</h1>

          <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
            <Input label="Email" type="email" placeholder="you@example.com" defaultValue="angel@example.com" />
            <Input label="Password" type="password" placeholder="••••••••" />

            <Link to="/dashboard">
              <Button fullWidth size="lg">
                Continue
              </Button>
            </Link>
          </form>

          <div className="my-6 flex items-center gap-4">
            <div className="h-px flex-1 bg-border" />
            <span className="text-xs font-medium uppercase text-text-muted">or</span>
            <div className="h-px flex-1 bg-border" />
          </div>

          <Button fullWidth variant="google" size="lg">
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path
                fill="currentColor"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="currentColor"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="currentColor"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="currentColor"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Continue with Google
          </Button>

          <p className="mt-8 text-center text-sm text-text-muted">
            Don&apos;t have an account?{' '}
            <Link to="/signup" className="font-medium text-primary hover:underline">
              Sign Up
            </Link>
          </p>
        </Card>
      </div>
    </div>
  )
}
